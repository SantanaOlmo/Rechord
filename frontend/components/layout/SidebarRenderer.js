    ...
    renderSongs(folderId, songs, suffix = '') {
        ...
        return songs.map((s, idx) => `
            <div class="group flex items-center pl-4 pr-2 py-1 hover:bg-gray-800 cursor-pointer text-gray-400 hover:text-white"
                 ...
                 <!-- Thumbnail -->
                 ${s.ruta_imagen
                ? `<img src="${CONTENT_BASE_URL}/${s.ruta_imagen}" class="w-6 h-6 rounded object-cover mr-2 bg-gray-800 flex-shrink-0" onerror="this.style.display='none'; this.parentNode.querySelector('.fallback-icon').style.display='flex'">`
                : ''}
                 <div class="fallback-icon w-6 h-6 rounded mr-2 bg-gray-800 flex items-center justify-center text-gray-500 flex-shrink-0 ${s.ruta_imagen ? 'hidden' : 'flex'}">
