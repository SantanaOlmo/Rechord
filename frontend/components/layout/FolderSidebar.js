...
export function FolderSidebar() {
    ...
    return `
        <div id="resizable-sidebar" class="bg-gray-950 flex flex-col w-full h-full overflow-hidden z-10 relative" 
            ...
            <!-- Context Menu -->
            <div id="folder-context-menu" class="fixed bg-gray-900 border border-gray-700 rounded shadow-xl hidden z-50 w-48 text-sm py-1">
                <button id="ctx-new-folder" class="w-full text-left px-4 py-2 text-gray-300 hover:bg-indigo-600 hover:text-white flex items-center hidden">
                ...
                <button id="ctx-add-song" class="w-full text-left px-4 py-2 text-gray-300 hover:bg-indigo-600 hover:text-white flex items-center hidden">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M12 4v16m8-8H4"></path></svg>
                    Añadir Canción
                </button>
                <button id="ctx-rename" class="w-full text-left px-4 py-2 text-gray-300 hover:bg-indigo-600 hover:text-white flex items-center hidden">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M11 5H6a2 2 0 00-2 2v11a2 2 0 002 2h11a2 2 0 002-2v-5m-1.414-9.414a2 2 0 112.828 2.828L11.828 15H9v-2.828l8.586-8.586z"></path></svg>
                    Renombrar
                </button>
                <div id="ctx-separator" class="border-t border-gray-700 my-1 hidden"></div>
                <button id="ctx-remove-song" class="w-full text-left px-4 py-2 text-red-400 hover:bg-red-900/50 flex items-center hidden">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    Quitar de carpeta
                </button>
                <button id="ctx-delete" class="w-full text-left px-4 py-2 text-red-400 hover:bg-red-900/50 flex items-center hidden">
                    <svg class="w-4 h-4 mr-2" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M19 7l-.867 12.142A2 2 0 0116.138 21H7.862a2 2 0 01-1.995-1.858L5 7m5 4v6m4-6v6m1-10V4a1 1 0 00-1-1h-4a1 1 0 00-1 1v3M4 7h16"></path></svg>
                    Eliminar Carpeta
                </button>
            </div>
...
async function initSidebar() {
    ...
    window.onBackgroundContextMenu = (e) => {
        if (e.target.closest('.folder-wrapper') || e.target.closest('.search-result-item')) return;
        e.preventDefault();
    ...
    window.onSongContextMenu = (e, folderId, songId) => {
        e.preventDefault(); e.stopPropagation();
        ctxSongId = songId;
        ctxSongFolderId = folderId;
    ...
    document.getElementById('ctx-add-song').onclick = () => {
    ...
    document.getElementById('ctx-remove-song').onclick = async () => {
